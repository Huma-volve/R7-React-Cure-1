...
const SpecialtiesCarousel: React.FC<SpecialtiesCarouselProps> = ({
    ...
    // Hide the entire section (including title) when there are no specialties
                    ...
                    <CarouselContent className="-ml-2 py-3 md:-ml-4">
                        {specialties.map((spec) => (
                            <CarouselItem key={spec.id} className="pl-2 basis-auto md:pl-4">
                                <Card
                                    className={cn(
                                        'cursor-pointer border transition-all hover:shadow-md rounded-2xl p-0 relative w-fit group',
                                        selectedSpecialty === spec.name
                                            ? 'border-(--bg-main) bg-(--bg-main) text-white'
                                            : 'border-gray-200'
                                    )}
                                ...
                                >
                                    <CardContent className="flex items-center gap-2 px-3 py-2 whitespace-nowrap">
                                        <img
                                            ...
                                            alt={spec.name}
                                            className={cn(
                                                'w-5 h-5 object-contain opacity-70 shrink-0 transition duration-300',
                                                selectedSpecialty === spec.name
